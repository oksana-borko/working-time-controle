...
export const checkRequestLimit = (reqList: Map<string, number[]>) => {
    return (req: AuthRequest, res: Response, next: NextFunction) => {
        ...
        const shouldLimitUser = userRoles?.some(role =>
            configuration.rateLimiting.enabledForRoles.includes(role)
        );
        ...
        if (!userRequests) {
            ...
            if (currTime - userRequests[0] > timeWindow) {
                ...
                throw new HttpError(429,
                    `Rate limit exceeded! Maximum ${maxRequests} requests per ${timeWindow/1000} seconds. Please wait.`
                );
            }
        }
